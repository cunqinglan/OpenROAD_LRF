...
/**
 * @brief Inserts a specific ABC mapping solution into the OpenROAD network.
 * 
 * This function takes a Map_MappingSolution_t from ABC's technology mapper,
 * applies it to the Map_Man_t to select specific cuts and gates, converts
 * the result to a mapped ABC netlist, and then inserts it into the OpenROAD
 * network by replacing the current cut instances.
 * 
 * @param pSolution The ABC mapping solution to apply (contains cut selections
 *                  and gate assignments for each node)
 * @param pMan The ABC mapping manager that contains all possible cuts and
 *             matches for each node
 * @param pOriginalNetwork The original AIG network that was used to create
 *                         the Map_Man_t (needed for Abc_NtkFromMap)
 * @param abc_library The ABC library containing gate information
 * @param network The OpenROAD network to insert the mapped solution into
 * @param unique_name Unique name generator for creating new instances
 * @param logger Logger for error reporting
 * 
 * @note This function is similar to InsertMappedAbcNetwork, but allows
 *       selecting from multiple mapping solutions generated by ABC's
 *       enumeration-based mapper.
 */
void LogicCut::InsertAbcMapSolution(abc::Map_MappingSolution_t* pSolution,
                                     abc::Map_Man_t* pMan,
                                     ...
                                     utl::Logger* logger)
{
  if (!pSolution) {
    logger->error(utl::CUT,
                  51,
                  "Map solution is NULL, please report this internal error.");
  }

  if (!pMan) {
    logger->error(utl::CUT,
                  52,
                  "Map manager is NULL, please report this internal error.");
  }

  if (!pOriginalNetwork) {
    logger->error(utl::CUT,
                  53,
                  "Original ABC network is NULL, please report this internal error.");
  }

  // Apply the solution to the mapping manager
  // This sets the best cuts for each node according to the solution
  abc::Map_MappingApplySolution(pMan, pSolution);
