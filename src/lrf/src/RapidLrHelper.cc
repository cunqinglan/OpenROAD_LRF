...
RapidLrHelper::updateEndPointArcLms(Edge *edge, TimingArc *arc, Sta *sta,
                                    Scene *scene, const MinMax *min_max)
{
  ...
  to_rat = std::max(delayAsFloat(to_rat), 0.0f);
                            ...
                            Scene *scene, const MinMax *min_max)
{
  ...
  sta::Arrival from_aat = sta->arrival(from_vertex, from_rf->asRiseFallBoth(), sta->scenes(), delay_minmax);
...
RapidLrHelper::updateCriticalPathLms(sta::Path *path_end)
{
  ...
  const int lib_ap = path_end->scene(sta_)->libertyIndex(path_end->minMax(sta_));
  const sta::DcalcAPIndex ap_index = path_end->dcalcAnalysisPtIndex(sta_);
