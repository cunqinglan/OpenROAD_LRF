    ...
    if (pg_rsz) {
      ...
      if (rb) {
        ...
        if (rb) {
          ...
          sta::Delay relaxation = std::max(sta::delayAsFloat(drvr_gate_delay), 0.0f) * 0.01f;
    ...
    if (pg_l) {  // process buf==0 too for baseline verification
      ...
      if (pv) {
        ...
        if (!vi.failed) {
          ...
          sta::DcalcAPIndex dap = sta_->findScene("default")
          ...
          for (size_t si = 0; si < pg_l->vertexCount(); si++) {
            ...
            if (spv.type() == PtVertexType::RefOutput
              ...
              PtPiElmore *pi = pg_l->findPtParasitic(
                  spv.objectIdx(), sta::RiseFall::rise(), dap);
              if (pi) {
                float pc2, prpi, pc1;
          ...
          if (g_bufs > 0) {
            ...
            for (sta::Vertex *lv : orig_sinks) {
              ...
              printf("        %-40s %+10.1f %+10.1f\n",
                     ...
                     sta::delayInf(arr, sta_) ? 0.0 : sta::delayAsFloat(arr) * 1e12,
                     s * 1e12);
              if (s < ws) ws = s;
              ss += s;
            }
