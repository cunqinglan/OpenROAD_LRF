...
LrRebuffer::annotateLoadSlacksSlackDp(BnetPtr& tree, sta::VertexId drvr_vid)
{
  ...
  sta::DcalcAPIndex target_dcalc_ap = eval_ctx_->pt_graph->apIndex();
  ...
  while (drvr_iter.hasNext()) {
    ...
    if (p->dcalcAnalysisPtIndex(local_sta_) == target_dcalc_ap) {
                           ...
                           float &max_slew)
{
  ...
  sta::DcalcAPIndex dcalc_pt = pt_graph->apIndex();
  sta::DcalcAPIndex ap_index = dcalc_pt;
  ...
  while (in_edge_iter.hasNext()) {
    ...
    for (sta::TimingArc *arc : edge->timingArcSet()->arcs()) {
      ...
      float ds = sta::delayAsFloat(dcalc_result.drvrSlew());
      if (ds > max_slew) {
        max_slew = ds;
      }
    }
...
LrRebuffer::computeBufferGateDelay(sta::LibertyCell *buffer_cell,
                                   float load_cap)
{
  sta::DcalcAPIndex dcalc_ap = eval_ctx_->pt_graph->apIndex();
  sta::LibertyPort *input, *output;
  buffer_cell->bufferPorts(input, output);
  sta::ArcDelay gate_delays[sta::RiseFall::index_count];
  sta::Slew slews[sta::RiseFall::index_count];
  resizer_->gateDelays(output, load_cap, eval_ctx_->pt_graph->scene(),
                       eval_ctx_->pt_graph->minMax(), gate_delays, slews);
    ...
    for (const sta::RiseFall *rf : sta::RiseFall::range()) {
      for (sta::Scene *corner : sta_->scenes())
        for (const sta::MinMax *min_max : sta::MinMax::range()) {
          pt_graph->setWireArcDelay(e, rf, corner->dcalcAnalysisPtIndex(min_max), d);
        }
    }
  };
                                      ...
                                      const VirtualBufferInfo &vinfo)
{
  ...
  Walker walk = [&](const BufferedNetPtr& bnet, VertexId current_drvr_id) {
    ...
    if (eval_ctx_->debug) {
      sta::DcalcAPIndex dap = sta_->findScene("default")->dcalcAnalysisPtIndex(sta::MinMax::max());
      PtPiElmore *pi_r = pt_graph->findPtParasitic(current_drvr_id, sta::RiseFall::rise(), dap);
      PtPiElmore *pi_f = pt_graph->findPtParasitic(current_drvr_id, sta::RiseFall::fall(), dap);
    ...
    for (sta::Scene *corner : sta_->scenes())
     for (const sta::MinMax *min_max : sta::MinMax::range()) {
      ...
      sta::Parasitics *parasitics = corner->parasitics(min_max);
      ...
      for (const sta::RiseFall *rf : sta::RiseFall::range()) {
        ...
        auto resistor_map = parasitics->parasiticNodeResistorMap(syn_net);
        ...
        elmoreDfs = [&](sta::ParasiticNode *node,
          ...
          if (it != resistor_map.end()) {
            for (sta::ParasiticResistor *res : it->second) {
              sta::ParasiticNode *onode = parasitics->otherNode(res, node);
              if (res != from_res && visited.find(onode) == visited.end()) {
                float r = parasitics->value(res);
...
LrRebuffer::repairSlew(const sta::Pin *drvr_pin, rsz::Resizer *resizer)
{
  ...
  while (remaining_cap > max_cap && loads.size() > 1) {
    ...
    int sum_x = 0, sum_y = 0;
    sta::dbNetwork *db_network = resizer->dbNetwork();
                                       ...
                                       PtVertex &drvr_pt_vertex)
{
  ...
  if (eval_ctx_->debug) {
    sta::DcalcAPIndex dap = sta_->findScene("default")->dcalcAnalysisPtIndex(sta::MinMax::max());
    for (const sta::RiseFall *rf : sta::RiseFall::range()) {
      PtPiElmore *pi = pt_graph->findPtParasitic(drvr_vid, rf, dap);
                                ...
                                float load_cap)
{
  ...
  for (sta::LibertyCell *buffer : swappable) {
    float slew = resizer->bufferSlew(
        buffer, load_cap, resizer->sta()->cmdScene(), sta::MinMax::max());
    if (slew < max_slew)
      return buffer;
    if (slew < min_slew) {
      min_slew = slew;
...
LrRebuffer::makeRepeater(rsz::Resizer *resizer,
                          ...
                          float &repeater_cap)
{
  sta::Network *network = resizer->dbNetwork();
