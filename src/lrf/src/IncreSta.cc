...
IncreSta::lmUpdate()
{
  ...
  if (lr_helper_ && lr_helper_->mode() != "power") {
    ...
    for (Clock *clock : sta_->cmdSdc()->clocks()) {
...
IncreSta::checkCapViolated(Pin *pin, const Scene *corner, const MinMax *min_max)
{
  ...
  sta_->checkCapacitance(pin, sta_->scenes(), min_max, capacitance, limit, slack, rf, corner1);
...
IncreSta::averageDelayOnCritPath() {
  ...
  Arrival worst_arrival = sta_->arrival(worst_vertex, sta::RiseFallBoth::riseFall(), sta_->scenes(), MinMax::max());
...
{
  sta::Scene *corner = sta_->findScene("default");
  sta::DcalcAPIndex dcalc_ap = corner->dcalcAnalysisPtIndex(MinMax::max());
  double sum = 0.0;
  int cnt = 0;
  sta::LeafInstanceIterator *inst_iter = network_->leafInstanceIterator();
  while (inst_iter->hasNext()) {
    ...
    while (pin_iter->hasNext()) {
      ...
      for (const RiseFall *rf : RiseFall::range()) {
        float sl = sta_->graph()->slew(vtx, rf, dcalc_ap);
        if (sl > s) s = sl;
      }
      sum += s;
      cnt++;
...
IncreSta::averageLoadCap()
{
  ...
  while (inst_iter->hasNext()) {
    ...
    while (pin_iter->hasNext()) {
      ...
      if (!network_->direction(pin)->isAnyOutput())
        continue;
      float lc = sta_->graphDelayCalc()->loadCap(pin, corner, MinMax::max());
      sum += lc;
      cnt++;
    }
    delete pin_iter;
      ...
      if (network->direction(pin)->isOutput()) {
        ...
        if (vertex) {
          ...
          if (slack < worst_slack)
            worst_slack = slack;
          float load = dcalc->loadCap(pin, corner, sta::MinMax::max());
          if (load > cout)
            cout = load;
        }
      }
