...
LocalSta::collectLocalVertices(Instance *inst, VertexSet &local_vertices)
{
  ...
  // The per-Mode clkNetwork is pre-built by the enclosing findRequireds();
  // isClock() is a const read, safe under the parallel workers.
  {
    sta::ClkNetwork *clk_network = sta_->cmdMode()->clkNetwork();
    InstancePinIterator *clk_iter = network_->pinIterator(inst);
    bool on_clock_network = false;
    while (clk_iter->hasNext()) {
      if (clk_network->isClock(clk_iter->next())) {
