      ...
      if (inst && graph) {
        ...
        if (in_pin) {
          ...
          if (in_v) {
            ...
            float s = graph->slew(in_v, in_rf, dcalc_ap);
            if (s > 0) in_slew = s;
          }
        }
      }
...
Initializer::run()
{
  ...
  sta::DcalcAPIndex sum_ap = corner->dcalcAnalysisPtIndex(max);
  ...
  for (odb::dbInst* db_inst : block_->getInsts()) {
    ...
    while (pit->hasNext()) {
      ...
      if (se) {
        ...
        if (v) {
          for (auto rf : RiseFall::range()) {
            float s = graph->slew(v, rf, sum_ap);
            if (s > sl) {
      ...
      if (network_->direction(p)->isOutput()) {
        ...
        if (ce) {
          float lc = sta_->graphDelayCalc()->loadCap(p, sta_->cmdScene(), sta::MinMax::max());
          if (lc > cl) {
            cap_cnt++;
            cap_sum += (lc - cl) * 1e15;
          }
...
Initializer::fixLoadViolations()
{
  ...
  const Scene* corner = sta_->cmdScene();
  const MinMax* max = MinMax::max();
  sta::DcalcAPIndex dcalc_ap = corner->dcalcAnalysisPtIndex(max);
  ...
  for (int i = static_cast<int>(sorted.size()) - 1; i >= 0; i--) {
    ...
    float load_cap = sta_->graphDelayCalc()->loadCap(out_pin, sta_->cmdScene(), sta::MinMax::max());
    ...
    const char* port_name = drvr_port->name().c_str();
...
Initializer::fixSlewViolations()
{
  ...
  for (int pass = 0; pass < 5; pass++) {
    ...
    for (auto& v : violations) {
      ...
      float load_cap = sta_->graphDelayCalc()->loadCap(v.drvr_pin, sta_->cmdScene(), sta::MinMax::max());
  ...
  while (viter2.hasNext()) {
    ...
    if (worst > limit) {
      ...
      float load_cap = sta_->graphDelayCalc()->loadCap(pin, sta_->cmdScene(), sta::MinMax::max());
      // Find largest equiv cell and its estimated slew
      LibertyCellSeq* equivs = cur ? sta_->equivCells(cur) : nullptr;
      const char* largest_name = cur ? cur->name().c_str() : "?";
      ...
      if (equivs && !equivs->empty()) {
        ...
        largest_name = largest->name().c_str();
...
Initializer::fixCapByBuffering()
{
  ...
  for (odb::dbInst* db_inst : block_->getInsts()) {
    ...
    while (pit->hasNext()) {
      ...
      float load_cap = sta_->graphDelayCalc()->loadCap(p, sta_->cmdScene(), sta::MinMax::max());
