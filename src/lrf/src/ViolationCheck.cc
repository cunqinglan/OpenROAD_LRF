                                ...
                                float &slack1) const
{
  if (!network_->isDriver(pin)) {
    ...
    if (net)
      pin = findNetParasiticDrvrPin(net);
    else
      return;
  }
                             ...
                             float &slack1) const
{
  float limit;
  bool limit_exists;
  localFindCapLimit(pin, lib_cell, corner, min_max, limit, limit_exists);
  if (limit_exists) {
    for (auto rf : sta::RiseFall::range()) {
      localCheckCapacitance(pin, lib_cell, corner, min_max, rf, limit,
...
void
LocalSta::localFindCapLimit(const sta::Pin *pin,
                const sta::LibertyCell *lib_cell,
                const sta::Corner *corner,
                const sta::MinMax *min_max,
                // Return values
                ...
                bool &exists) const
{
  ...
  sdc_->capacitanceLimit(top_cell, min_max,
			limit, exists);
  ...
  if (network->isTopLevelPort(pin)) {
    ...
    sta::Cell *cell = network_->cell(const_cast<sta::LibertyCell*>(lib_cell));
    sdc_->capacitanceLimit(cell, min_max,
			  limit1, exists1);
    if (exists1
	&& (!exists
	    || min_max->compare(limit, limit1))) {
    ...
    LibertyPort *port = lib_cell->findLibertyPort(network_->portName(pin));
    if (port) {
...
{
  float load_cap = 0.0;
  connectedCap(pin, rf, corner, min_max, load_cap);
  float slack = (min_max == sta::MinMax::max()) ? (limit - load_cap) : (load_cap - limit);
  if (slack < slack1
      || (fuzzyEqual(slack, slack1) 
      && rf->index() < rf1->index())) {
    corner1 = corner;
    rf1 = rf;
    capacitance1 = load_cap;
    slack1 = slack;
    limit1 = limit;
  }
}
...
void
LocalSta::connectedCap(const Pin *drvr_pin,
                      const sta::RiseFall *rf,
                      const sta::Corner *corner,
                      const sta::MinMax *min_max,
