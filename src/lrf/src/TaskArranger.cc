...
TaskArranger::init()
{
  if (vertices_.empty()) {
    ...
    modes()[0]->sim()->ensureConstantsPropagated();
    makeGraph();
...
TaskArranger::hasUsableDriver(sta::Instance *inst)
{
  ...
  while (it->hasNext()) {
    ...
    if (v && pred.searchTo(v, modes()[0])) { any = true; break; }
  }
  delete it;
  return any;
}
...
TaskArranger::makeVertices()
{
  ...
  // Create vertex for top instance
  {
    ...
    std::string inst_name = network_->name(top_inst);
                        ...
                        InstVertex* to_vertex)
{
  ...
  if (to_vertex->type() == VertexType::SEQUENTIAL
    ...
    std::string from_name = from_vertex->inst() ? network_->name(from_vertex->inst()) : std::string("<null>");
    std::string to_name = to_vertex->inst() ? network_->name(to_vertex->inst()) : std::string("<null>");
