...
{
  // Hold shared lock during evaluation to prevent concurrent DB modification
  // by applyChangesToDb() on neighbor instances.
  std::shared_lock<std::shared_mutex> read_lock(g_odb_sta_access_mutex);
  bool success;
...
{
  // Exclusive lock: waits for all shared_lock readers (visit) to finish.
  std::unique_lock<std::shared_mutex> write_lock(g_odb_sta_access_mutex);
  switch (move_type_) {
    case MoveType::Resizing:
